...
function cb_cookies_bend(){
	...
	if ( !file_exists($log_file) ) {
		...
		$cdata = $thislog->addChild('cbcook_msg',$ndVal[0]);   //ndVal=0 text of messagex (x=0,1,2 in file lang.php)
...
<!--
       		function ncselect(id,texto){
					...
					var select=document.getElementById("selmsg");
					frm.value = select.options[select.selectedIndex].text;
					var txtar= document.getElementById("vlmsg");
					txtar.value=texto;
	        }	            
-->
	</script>
<?php
	...
	for ($q=0; $q<3; $q++){
		if ($q == $ndVal[0]){ 
			$optionmsg.='<OPTION VALUE="'.$q.'" SELECTED>'.i18n_r("cbcookies/message$q").'</OPTION>'."\n";
		} else {
			$optionmsg.='<OPTION VALUE="'.$q.'">'.i18n_r("cbcookies/message$q").'</OPTION>'."\n";
		}
	}
	...
	echo '<form name="formulario" action="load.php?id=cbcookies&amp;action=setting&amp;sett=y" method="post">'."\n";
		...
		echo '<div style="float: left; width: 95%;">'."\n";
	        	echo '<p><label>'.i18n_r('cbcookies/message_lb').':</label>'."\n";
           		echo '<SELECT id="selmsg" NAME="sett[0]" style="border: 1px solid #AAAAAA; border-radius: 2px 2px 2px 2px; text-align: left; padding-right: 1px; width: 90%;" onclick="javascript:ncselect(&quot;txtmsg&quot;,this.value)">'."\n";
        	        	echo $optionmsg;
        	   	echo '</SELECT></p>'."\n";
	        	echo '<p><textarea disabled="true" id="txtmsg" style="border: 1px solid #AAAAAA; display:none;height:300px;border-radius: 2px 2px 2px 2px; height: 40px; margin: 0 0 0 3px; width: 100%; text-align: left;" type="text" name="q_set">'.i18n_r("cbcookies/message$ndVal[0]").'</TEXTAREA></p>'."\n";
	        	echo '<p><textarea id="vlmsg" style="display:none;" type="text" name="q_sett[0]">'.$ndVal[0].'</textarea></p>'."\n";
		echo '</div>'."\n";
...
function cb_cookies() {
	if (!isset($_COOKIE['acceptco'])){
		...
		//i18n lang  
		$LANG = (!file_exists(GSPLUGINPATH.'cbcookies/lang/'.$LANG.'.php')) ? 'en_US' : $LANG;
		i18n_merge('cbcookies', $LANG); 	
		$log_name = 'cbcookies.xml';
		$log_path = GSDATAOTHERPATH.'cbcookies/';
		$log_file = $log_path . $log_name;
		...
		if (file_exists($log_file)) {
			...
			$xml = new SimpleXMLExtended('<?xml version="1.0" encoding="UTF-8"?><entry></entry>');
			$thislog = $xml->addChild('sett');
			$cdata = $thislog->addChild('cbcook_msg','0');//ndVal=0 text of messagex(x=0,1,2  in LANG.php)
...
<?php
		...
		echo '<div id="'.$cbcookies_w.'" style="display: block;">'.i18n_r("cbcookies/message$cbmessage").'&nbsp;&nbsp;<a id="cbcookies_accept" href="javascript:acceptco(&quot;'.$cbcookies_w.'&quot;, &quot;cbcookies&quot;)" >'.$i18n['cbcookies/accept'].'</a>&nbsp;&nbsp;';
		if (trim($cbpage) != ''){
			$P_lnk = find_url($cbpage,'');
			echo '<a id="cbcookies_more" href="'.$P_lnk.'">'.$i18n['cbcookies/more_in'].'</a>';
		}
		echo '</div>';
	} //End if (!isset($_COOKIE[acceptco]))
